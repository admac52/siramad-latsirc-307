...
int main(){
	...
	float DIA[100][100],DIB[100][100],ID[100][100];
	...
	if(adia==1){cout<<"\n\t********** OPERACIONES ADICIONALES **********"<<endl;
		...
		switch(extra){
			...
			case 7:{cout<<"\t******** MATRIZ DIAGONAL ********"<<endl<<endl;
				if(m1==n1){
					if((DETA(MA, m1))!=0){
						float a=1;
						float b=-MA[0][0]-MA[1][1];
						float c=(MA[0][0]*MA[1][1])-(MA[0][1]*MA[1][0]);
						...
						if(disc<0){
							float conv=(-1)*disc;
							float raiz=sqrtf(conv);
							float real1=(-b)/(2*a);
							float im1=(raiz)/(2*a);
							float real2=(-b)/(2*a);
							float im2=(-raiz)/(2*a);
							cout<<"\nLas raices del polinomio caracteristico son:"<<endl<<endl;
							if(im1>=0){
					...
					}else{cout<<"\nNO SE PUEDE DIAGONALIZAR, SU DETERMINANTE ES CERO"<<endl;
					}
				}else{cout<<"\nLA MATRIZ A NO ES CUADRADA"<<endl;
			...
			case 9:{cout<<"\t******** FACTORIZACION LU ********"<<endl<<endl;
				for(int i=0;i<m1;i++){cout<<"\t";
					for(int j=0;j<n1;j++){
						if(i==j){
							LA[i][j]=1;
			...
			case 10:{cout<<"\t******** FACTORIZACION CHOLESKY ********"<<endl<<endl;
				if(m1==n1){a=0;
					for(int i=0;i<m1;i++){
						for(int j=0;j<n1;j++){
							LA[i][j]=0;
					...
					if(a==(m1*n1)){cout<<"LA MATRIZ A ES SIMETRICA, POR TANTO"<<endl<<endl;
						for(int i=0;i<m1;i++){
							for(int j=0;j<=i;j++){
								float sum=0;
								if(j==i){
			...
			case 11:{cout<<"\t******** VALORES Y VECTORES PROPIOS ********"<<endl<<endl;
					for (int i=0;i<m1;i++){
						for (int j=0;j<n1;j++){
							if(i==j){ID[i][j]=1;
							}else{ID[i][j]=0;
					...
					for (int i=0;i<m1;i++){
						for (int j=0;j<n1;j++){
							DIA[i][j]=MA[i][j]-(ID[i][j]);	
						}
					}
					cout<<"VALOR PROPIO 1"<<endl;
					cout<<"\t"<<-DIA[0][1]<<endl;
					cout<<"\t"<<DIA[0][0]<<endl;
	...
	if(adib==1){cout<<"\t********** OPERACIONES ADICIONALES **********"<<endl;
		...
		switch(extra){
			...
			case 4:{cout<<"\t******** MULTIPLICAR POR UN ESCALAR ********"<<endl<<endl;
			...
			case 6:{cout<<"\t******** MATRIZ TRIANGULAR ********"<<endl<<endl;
			...
			case 7:{cout<<"\t******** MATRIZ DIAGONAL ********"<<endl<<endl;
				if(m1==n1){
					if((DETA(MB, m2))!=0){
						float a=1;
						float b=-MB[0][0]-MB[1][1];
						float c=(MB[0][0]*MB[1][1])-(MB[0][1]*MB[1][0]);
						float disc=(b*b)-(4*a*c);
			...
			case 10:{cout<<"\t******** FACTORIZACION CHOLESKY ********"<<endl<<endl;
				if(m2==n2){a=0;
					...
					if(a==(m2*n2)){cout<<"LA MATRIZ A ES SIMETRICA, POR TANTO"<<endl;
						for(int i=0;i<m2;i++){
							for(int j=0;j<=i;j++){
								float sum=0;
								if(j==i){
